...
class SubmissionTests(unittest.TestCase):
    ...
    def test_malformed_cells_are_not_silently_coerced(self):
        ...
        _parsed, missing = script.parse_judgment(json.dumps(obj))
        self.assertIn("__top_level__", missing)
    ...
    def test_incomplete_json_retries_then_fails_loudly(self):
        class AlwaysPartial:
            load_seconds = 0.0
            ...
            def chat(self, batch):
                return ['{"v1":{"y":0,"s":-1}}' for _ in batch]

        with tempfile.TemporaryDirectory() as tmp:
            input_path = Path(tmp) / "input.jsonl.gz"
            with gzip.open(input_path, "wt", encoding="utf-8") as handle:
                handle.write(json.dumps(make_record(), ensure_ascii=False) + "\n")
            with self.assertRaisesRegex(RuntimeError, "2회 호출"):
                script.run(str(input_path), str(Path(tmp) / "submission.csv"),
                           AlwaysPartial, None, 8, 4000, str(DATA))
