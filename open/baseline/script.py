...
def parse_judgment(text: str) -> Tuple[Dict[str, Dict[str, int]], List[str]]:
    ...
    missing: List[str] = []
    if isinstance(obj, dict) and set(obj) != set(ITEMS):
        missing.append("__top_level__")
    for item in ITEMS:
        raw = obj.get(item) if isinstance(obj, dict) else None
        valid = (
            isinstance(raw, dict) and set(raw) == {"y", "s"} and
            type(raw.get("y")) is int and raw["y"] in (0, 1) and
            type(raw.get("s")) is int and -1 <= raw["s"] <= 512
        )
        if not valid:
            missing.append(item)
            output[item] = {"y": 0, "s": -1}
        else:
            output[item] = {"y": raw["y"], "s": raw["s"]}
...
def run(input_path: str, out_path: str, runner_cls: Any, limit: Optional[int],
    ...
    runner = runner_cls(schema, **runner_kwargs)
    ...
    if invalid_indices:
        log(f"불완전 compact JSON {len(invalid_indices)}건 재호출")
        retried = run_chunk(runner, [messages_all[i] for i in invalid_indices])
        for index, text in zip(invalid_indices, retried):
            parsed_all[index] = parse_judgment(text)

    still_invalid = [(records[i]["id"], missing) for i, (_parsed, missing)
                     in enumerate(parsed_all) if missing]
    if still_invalid:
        sample = ", ".join(f"{rec_id}({len(missing)}개 누락)"
                           for rec_id, missing in still_invalid[:8])
        raise RuntimeError(f"2회 호출 뒤에도 구조화 출력 불완전: {sample}")
    ...
    if labels_path:
        report["dev평가"] = evaluate_rows(rows, labels_path)
...
def main() -> int:
    ...
    if min(args.chunk, args.max_chars, args.max_tokens) <= 0:
        parser.error("chunk/max-chars/max-tokens는 양수여야 합니다")
    if args.max_tokens >= MAX_MODEL_LEN:
        parser.error("max-tokens는 모델 컨텍스트보다 작아야 합니다")
